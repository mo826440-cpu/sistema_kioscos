// =====================================================
// AGREGAR CAMPO ESTADO A CLIENTES
// =====================================================
...
async function addEstadoField() {
  console.log('🔧 AGREGANDO CAMPO ESTADO A CLIENTES')
  console.log('============================================================')
  ...
  try {
    ...
    const tableInfo = db.exec('PRAGMA table_info(clientes)')
    const hasEstado = tableInfo[0].values.some(row => row[1] === 'estado')
    
    if (hasEstado) {
      console.log('✅ El campo "estado" ya existe en la tabla clientes')
      console.log('   No es necesario hacer cambios')
      return
    }
    
    console.log('📝 Agregando campo "estado" a la tabla clientes...')
    
    // Agregar el campo estado (por defecto 'activo')
    db.run(`ALTER TABLE clientes ADD COLUMN estado TEXT DEFAULT 'activo'`)
    
    // Actualizar todos los clientes existentes a 'activo'
    db.run(`UPDATE clientes SET estado = 'activo' WHERE estado IS NULL`)
    
    console.log('✅ Campo agregado exitosamente')
    
    // Verificar
    const countResult = db.exec('SELECT COUNT(*) as total FROM clientes')
